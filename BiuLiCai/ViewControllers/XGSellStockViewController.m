...
@interface XGSellStockViewController ()
...
@implementation XGSellStockViewController
+ (instancetype)getInstance
{
    return [[UIStoryboard storyboardWithName:@"XGSimulateStockViewController" bundle:nil] instantiateViewControllerWithIdentifier:@"XGSellStockViewController"];
}

- (void)viewDidLoad {
    [super viewDidLoad];
    [((XGMainViewController *)self.navigationController.parentViewController) hideTabBar];
    [self.sellButton setBackgroundImage:[UIImage imageWithColor:[UIColor hexColor:0xfc594b]] forState:UIControlStateNormal];
    self.sellButton.layer.masksToBounds = YES;
    self.sellButton.layer.cornerRadius = self.sellButton.height / 2;
...
-(void)requestData
{
    ...
    [[XGHttpManager sharedManager] getSellOutInfo:@"600010" successBlock:^(id responseObject) {
...
-(void)loadData:(id)data
{
    ...
    self.maxBuyAmount.text = [NSString stringWithFormat:@"最大可卖%ld",self.maxBuy];
